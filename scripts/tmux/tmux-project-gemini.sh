...
# ========================================
# Universal Tmux Development Environment (Gemini Version)
# Usage: ./tmux-project-gemini.sh [project-directory]
...
echo "========================================"
echo " Universal Dev Environment (Gemini)"
echo "========================================"
echo ""
...
check_command "gemini" || echo "Warning: gemini not found"
...
setup_window() {
    ...
    # Pane 1: Gemini (top-left)
    tmux send-keys -t $SESSION_NAME:"$window_name".1 "clear && echo '=== $window_name - Gemini ===' && echo 'Role: $role_description' && echo ''" Enter
    if command -v gemini &> /dev/null; then
        tmux send-keys -t $SESSION_NAME:"$window_name".1 "gemini" Enter
    else
        tmux send-keys -t $SESSION_NAME:"$window_name".1 "echo 'gemini not installed'"
    fi
...
# Select first window and Gemini pane
