...
# ========================================
# Universal Tmux Development Environment (Qwen Version)
# Usage: ./tmux-project-qwen.sh [project-directory]
...
echo "========================================"
echo " Universal Dev Environment (Qwen)"
echo "========================================"
echo ""
...
check_command "qwen" || echo "Warning: qwen not found"
...
setup_window() {
    ...
    # Pane 1: Qwen (top-left)
    tmux send-keys -t $SESSION_NAME:"$window_name".1 "clear && echo '=== $window_name - Qwen ===' && echo 'Role: $role_description' && echo ''" Enter
    if command -v qwen &> /dev/null; then
        tmux send-keys -t $SESSION_NAME:"$window_name".1 "qwen" Enter
    else
        tmux send-keys -t $SESSION_NAME:"$window_name".1 "echo 'qwen not installed'"
    fi
...
# Select first window and Qwen pane
