...
# ========================================
# Universal Tmux Development Environment (Kimi Version)
# Usage: ./tmux-project-kimi.sh [project-directory]
...
echo "========================================"
echo " Universal Dev Environment (Kimi)"
echo "========================================"
echo ""
...
check_command "kimi" || echo "Warning: kimi not found"
...
setup_window() {
    ...
    # Pane 1: Kimi (top-left)
    tmux send-keys -t $SESSION_NAME:"$window_name".1 "clear && echo '=== $window_name - Kimi ===' && echo 'Role: $role_description' && echo ''" Enter
    if command -v kimi &> /dev/null; then
        tmux send-keys -t $SESSION_NAME:"$window_name".1 "kimi" Enter
    else
        tmux send-keys -t $SESSION_NAME:"$window_name".1 "echo 'kimi not installed'"
    fi
...
# Select first window and Kimi pane
